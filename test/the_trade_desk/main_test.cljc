(ns the_trade_desk.main-test
  "Contract + behavioral test for the the_trade_desk-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "the_trade_desk-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'the_trade_desk.main-test)]
