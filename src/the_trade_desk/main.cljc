(ns the_trade_desk.main
  "Kotodama WASM entrypoint for the the_trade_desk clean-room actor (L4) — Clojure port.
  ...
  facts (`the_trade_desk.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "the_trade_desk")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"the_trade_desk.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "the_trade_desk-compat" :tier tier :entities entities} 200])
...
(defn start! [] :the_trade_desk-compat/ready)
